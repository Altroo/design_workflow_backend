...
class ProjectDetailSerializer(ProjectSummarySerializer):
    ...
    def get_tasks(self, obj):
        tasks = obj.tasks.select_related("project__manager", "current_assignee")
    ...
    def get_contributors(self, obj):
        contributor_ids = set(
            TaskComment.objects.filter(task__project=obj).values_list("author_id", flat=True)
        )
        contributor_ids.update(
            TimeEntry.objects.filter(task__project=obj).values_list("user_id", flat=True)
        )
        contributor_ids.update(
            Task.objects.filter(project=obj, current_assignee_id__isnull=False).values_list(
                "current_assignee_id", flat=True
            )
        )
        contributors = User.objects.filter(id__in=contributor_ids).order_by("first_name", "last_name")
        return UserSummarySerializer(contributors, many=True).data
...
class ProjectWriteSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        target_end_date = attrs.get(
            "target_end_date",
            getattr(self.instance, "target_end_date", None),
        )
